...
def run(PN_reac, storage_time, profile):
    ...
    (Time, P_core, P_load, P_unload, stored_energy) = load_following(P_grid, reac, hot_tank, cold_tank, max_stored_energy, P_unload_max)
    ...
    print_load_graph(P_grid, reac, hot_tank, cold_tank, max_stored_energy, P_unload_max, Time, P_core, P_load, P_unload, stored_energy, 0, len(P_grid)-1)
